...
async function prepareCarouselProductsForRule({
  ...
  async function buildLockedCampaignSearchPool({
    ...
    try {
      ...
      const lockableSearchPoolItems = freshSafeStoreSearchPoolItems.length >= CAMPAIGN_LOCKED_SEARCH_POOL_MIN_ITEMS
        ? freshSafeStoreSearchPoolItems
        : safeStoreSearchPoolItems;

      if (lockableSearchPoolItems.length >= CAMPAIGN_LOCKED_SEARCH_POOL_MIN_ITEMS) {
        lockedCampaignSearchPoolItems = lockableSearchPoolItems;
...
async function prepareWebsiteContentForRule({
  ...
  if (isCampaignScopedWebsiteRule(rule) && catalogItems.length) {
    catalogItems = await applyAiCampaignFitScores({
      openai,
      rule,
      brandProfile,
  ...
  try {
    ...
    if (discoveredCandidates.length) {
      ...
      if (isCampaignScopedWebsiteRule(rule) && discoveredItems.length) {
        discoveredItems = await applyAiCampaignFitScores({
          openai,
          rule,
          brandProfile,
          items: discoveredItems,
          maxItems: WEBSITE_PRODUCT_DISCOVERY_VERIFY_LIMIT,
        });
      }
