...
export function buildProductPushEdit({
  ...
  const safeProductWidth = Math.max(
    240,
    Math.min(920, Math.round(Number(productWidth) || 760))
  );
  const safeProductHeight = Math.max(
    240,
    Math.min(920, Math.round(Number(productHeight) || 860))
  );
  // Shotstack's vertical offset uses viewport height and positive values move
  // upwards. A negative value moves the top-positioned product down.
  const productTopOffset = -(255 / 1920);
  const tracks = [];
