...
export function buildProductPushEdit({
  ...
  const safeWidth = Math.max(280, Math.min(860, Math.round(Number(productWidth) || 760)));
  const safeHeight = Math.max(280, Math.min(980, Math.round(Number(productHeight) || 860)));
  ...
  const productJs = [
    ...
    `tl.to('#product-motion',{scale:1.07,duration:${half},ease:'sine.inOut'});`,
