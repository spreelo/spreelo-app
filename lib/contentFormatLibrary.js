...
export function normalizeContentFormatRows(rows = []) {
  ...
  return DEFAULT_CONTENT_FORMAT_LIBRARY.map((defaults) => ({
    ...
    image_url: storedMap[defaults.content_type_id]?.image_url || null,
    image_storage_path: storedMap[defaults.content_type_id]?.image_storage_path || null,
    icon_url: storedMap[defaults.content_type_id]?.icon_url || null,
    icon_storage_path: storedMap[defaults.content_type_id]?.icon_storage_path || null,
  }));
}
