...
export function resolvePostRescueType(source = {}) {
  ...
  const usesWebsiteContent = Boolean(
    source?.uses_website_content ??
      source?.usesWebsiteContent ??
      source?.work_item?.uses_website_content ??
      source?.work_item?.usesWebsiteContent
  );
  const imageSource = normalize(source?.image_source || source?.work_item?.image_source);
  ...
  if (contentType === "problem_solution" && usesWebsiteContent) {
    return POST_RESCUE_TYPES.SINGLE_PRODUCT;
  }
  if (
    contentType === "engagement_humor" &&
    usesWebsiteContent &&
    (imageSource === "website" || format === "animated_video")
  ) {
    return POST_RESCUE_TYPES.SINGLE_PRODUCT;
  }
