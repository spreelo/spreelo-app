...
export async function sendSocialConnectionAlertEmail({
  ...
  const subject = `WARNING: Spreelo social publishing requires attention (${platformLabel})`;
  ...
  const html = `
    <div style="font-family:Arial,sans-serif;line-height:1.5;color:#111827;max-width:680px;margin:0 auto;padding:24px;">
      <div style="border:1px solid #fecaca;background:#fff7f7;border-radius:16px;padding:20px;">
        <h1 style="margin:0 0 12px;font-size:22px;color:#991b1b;">Internal operations warning: social publishing requires attention</h1>
        <p style="margin:0 0 16px;">Spreelo detected that a ${escapeHtml(platformLabel)} connection no longer works for publishing or token renewal. This email is sent only to Spreelo administrators, not to the customer.</p>
        <table style="width:100%;border-collapse:collapse;font-size:14px;">
          <tr><td style="padding:6px 0;color:#6b7280;">Platform</td><td style="padding:6px 0;font-weight:700;">${escapeHtml(platformLabel)}</td></tr>
          <tr><td style="padding:6px 0;color:#6b7280;">Affected account/page</td><td style="padding:6px 0;">${escapeHtml(safePageName)}</td></tr>
          ...
          <tr><td style="padding:6px 0;color:#6b7280;">Time</td><td style="padding:6px 0;">${escapeHtml(createdAt)}</td></tr>
        </table>
        <p style="margin:16px 0 4px;color:#6b7280;">Reason</p>
        ...
        <p style="margin:18px 0 0;">Check the platform token, permissions, app settings and the affected social connection in Spreelo before the customer is affected further.</p>
        <p style="margin:14px 0 0;"><a href="${escapeHtml(APP_URL)}/social-channels" style="display:inline-block;background:#111827;color:white;text-decoration:none;padding:10px 14px;border-radius:999px;font-weight:700;">Open Social channels</a></p>
      </div>
    </div>
  `;

  const text = [
    `WARNING: Spreelo social publishing requires attention (${platformLabel})`,
    `Platform: ${platformLabel}`,
    `Affected account/page: ${safePageName}`,
    `Page/IG ID: ${pageId || "-"}`,
    `User ID: ${userId || "-"}`,
    ...
    `Internal admin link: ${APP_URL}/social-channels`,
  ].join("\n");
