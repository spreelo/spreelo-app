...
export function campaignHasProductWebsiteFit(campaign = {}) {
  ...
  // A brand-level capability is based on technically verified products and is
  // authoritative for format availability. Campaign rows created by older or
  // imperfect analyses may contain blank/weak/none strategy metadata; that
  // metadata must not turn an approved product store into an all-AI-image plan.
  // Explicit service campaigns still use the service branch in the planner.
  if (strategy === "service") {
    return false;
  }
