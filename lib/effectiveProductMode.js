...
import { fetchShopifyProductEngineCatalog } from "./shopifyProductCatalog.js";
...
function buildShopifyProductModeReason(catalog) {
  ...
  return `Verified from the connected Shopify Admin API: ${eligibleCount} active Online Store product${eligibleCount === 1 ? "" : "s"} with an available-for-sale variant and product image.`;
}
