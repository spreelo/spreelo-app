...
test("Unicode and non-Swedish campaign themes keep a stable shared fingerprint", () => {
  const first = buildCampaignFingerprint({
    theme: ["halloween'i kampaania", "maagilised pardid"],
    match: ["kummipart", "pühadepardid"],
    avoid: ["jõulud"],
    intent: "hooajaline kingitus",
    need: "lõbus halloweeni dekoratsioon",
    fallback: "halloween kummipardid",
  });
  const second = buildCampaignFingerprint({
    theme: ["maagilised pardid", "halloween'i kampaania"],
    match: ["pühadepardid", "kummipart"],
    avoid: ["jõulud"],
    intent: "hooajaline kingitus",
    need: "lõbus halloweeni dekoratsioon",
    fallback: "halloween kummipardid",
...
test("exact products for any language beat generic next-best products", () => {
  const exactForeignThemeProducts = Array.from(
    { length: 6 },
    (_, index) => candidate(`maagiline-part-${index}`, 0, 0, 100)
  );
  const genericProducts = Array.from(
    { length: 20 },
    (_, index) => candidate(`generic-${index}`, 2, 0, 70)
  );

  const selected = chooseQualityCutoffAndRank(
    [...genericProducts, ...exactForeignThemeProducts],
    5
  );

  assert.ok(selected.every((item) => item.id.startsWith("maagiline-part-")));
});
