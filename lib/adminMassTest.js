...
export function getAdminMassTestFormats(formatRows = []) {
  ...
  return DEFAULT_CONTENT_FORMAT_LIBRARY.map((item) => {
    ...
    return {
      ...
      label: item.content_type_id === "carousel_website_item"
        ? "AI-designed carousel – 5 products"
        : row.display_label || item.default_label,
      description: item.content_type_id === "carousel_website_item"
        ? "Five verified products in five coordinated AI-designed slides. Slide five also carries the closing CTA."
