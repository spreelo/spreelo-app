...
export function canonicalizeStoreMapUrl(value, baseUrl = "") {
  try {
    ...
    for (const key of Array.from(url.searchParams.keys())) {
      if (/^(?:utm_|fbclid|gclid|mc_|ref|source|campaign)/i.test(key)) {
...
export function getStoreMapAgentTargets(requiredCount = 5) {
  ...
  return {
    ...
    minimumVerifiedProducts: isCarouselScale
      ? Math.max(12, count * 2 + 2)
      : Math.max(4, count + 3),
    mapCrawlPageLimit: isCarouselScale ? Math.max(18, count * 4) : 10,
    mapDepthLimit: 3,
    shelfSelectionLimit: isCarouselScale
      ? Math.max(6, Math.min(10, count + 3))
      : 4,
    shelfProductLimit: isCarouselScale ? Math.max(24, count * 6) : 12,
    mapNodeLimit: 600,
  };
}
