...
const INTERNAL_WORKFLOW_QUERY_PATTERNS = [
  ...
  /^with\s+(?:usable|verified|candidate|candidates|product|products|item|items|count)\b/i,
...
const INTERNAL_WORKFLOW_QUERY_TOKENS = new Set([
  "with",
  "without",
  "verified",
  "usable",
...
export function isSafeProductSearchQuery(value) {
  ...
  // Internal orchestration fragments sometimes arrive as tiny standalone
  // search phrases after AI metadata has been split on punctuation. Reject
  // those fragments before they can become retailer searches such as
  // "verified", "with" or "with usable". Real product phrases that happen to
  // contain one of these words remain valid as long as they also contain
  // genuine product vocabulary.
  if (words.every((word) => INTERNAL_WORKFLOW_QUERY_TOKENS.has(word))) return false;
...
function normalizeVariantEvidenceText(value) {
...
function normalizeMetricVariantValue(rawValue, rawUnit) {
  const value = Number(String(rawValue || "").replace(",", "."));
  if (!Number.isFinite(value)) return null;
  const unit = String(rawUnit || "").toLowerCase().replace(/\s+/g, "");

  if (["kg", "kilo", "kilos", "kilogram", "kilograms"].includes(unit)) {
    return { dimension: "mass_g", value: Math.round(value * 1000 * 1000) / 1000 };
  }
  if (["g", "gr", "gram", "grams"].includes(unit)) {
    return { dimension: "mass_g", value: Math.round(value * 1000) / 1000 };
  }
  if (["l", "liter", "litre", "liters", "litres"].includes(unit)) {
    return { dimension: "volume_ml", value: Math.round(value * 1000 * 1000) / 1000 };
  }
  if (["dl"].includes(unit)) {
    return { dimension: "volume_ml", value: Math.round(value * 100 * 1000) / 1000 };
  }
  if (["cl"].includes(unit)) {
    return { dimension: "volume_ml", value: Math.round(value * 10 * 1000) / 1000 };
  }
  if (["ml"].includes(unit)) {
    return { dimension: "volume_ml", value: Math.round(value * 1000) / 1000 };
  }
  return null;
}

export function extractComparableProductVariantFacts(value) {
  const text = normalizeVariantEvidenceText(value);
  const facts = {};
  if (!text) return facts;

  const packMatch = text.match(/\b(\d{1,3})\s*(?:pack|pk|pcs|pieces|st|styck(?:en)?|count)\b/i);
  if (packMatch) facts.pack_count = Number(packMatch[1]);

  const metricRegex = /\b(\d{1,5}(?:[.,]\d{1,3})?)\s*(kg|kilo(?:s)?|kilograms?|g|gr|grams?|l|liters?|litres?|dl|cl|ml)\b/gi;
  for (const match of text.matchAll(metricRegex)) {
    const normalized = normalizeMetricVariantValue(match[1], match[2]);
    if (!normalized || facts[normalized.dimension] != null) continue;
    facts[normalized.dimension] = normalized.value;
  }

  return facts;
}

export function getDeterministicProductImageVariantConflict({
  productTitle = "",
  imageUrl = "",
} = {}) {
  const expected = extractComparableProductVariantFacts(productTitle);
  const observed = extractComparableProductVariantFacts(imageUrl);
  const conflicts = [];

  for (const dimension of ["pack_count", "mass_g", "volume_ml"]) {
    if (expected[dimension] == null || observed[dimension] == null) continue;
    if (Number(expected[dimension]) === Number(observed[dimension])) continue;
    conflicts.push({
      dimension,
      expected: Number(expected[dimension]),
      observed: Number(observed[dimension]),
    });
  }

  // Filenames/CDN paths can occasionally be stale. One differing number alone
  // is therefore only a warning signal. Two independent variant dimensions
  // (for example 5-pack + 290 g versus 6-pack + 350 g) are strong enough to
  // fail closed before spending a semantic-vision call on a suspicious asset.
  ...
  return {
    ...
    expected,
    observed,
    reason: "deterministic_image_url_variant_conflict",
  };
}
