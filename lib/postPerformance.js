...
export async function collectProviderMetrics({ platform, accessToken, externalIds, connection, publishedAt, now = new Date() }) {
  switch (String(platform || "").toLowerCase()) {
    case "facebook":
      return fetchFacebookPostMetrics({ accessToken, externalIds });
    case "instagram":
      return fetchInstagramPostMetrics({ accessToken, externalIds });
    case "pinterest":
      return fetchPinterestPostMetrics({ accessToken, externalIds, publishedAt, now });
    case "tiktok":
      return fetchTikTokPostMetrics({ accessToken, externalIds, connection });
    case "youtube":
      return fetchYouTubePostMetrics({ accessToken, externalIds });
    default:
