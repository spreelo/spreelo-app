...
function normalizeServiceMode(rawMode, websiteUrl = "", productMode = null) {
  const mode = rawMode && typeof rawMode === "object" ? rawMode : {};
  const legacyType = String(productMode?.source_type || "").trim().toLowerCase();
  const legacyVerified = Boolean(productMode?.available) && ["service_catalog", "booking", "course_event"].includes(legacyType);
  const available = Boolean(mode.available) || legacyVerified;
  return {
    available,
    reason: cleanRescueText(mode.reason, 500) || (legacyVerified ? "Verified from the legacy website service/catalog evidence." : ""),
    source_url: cleanRescueUrl(mode.source_url) || (available ? cleanRescueUrl(productMode?.source_url || websiteUrl) : ""),
  };
}
...
export function validateAdminRescueManifest({ manifest, rescueCase, brand }) {
  ...
  return {
    ...
    website_service_mode: normalizeServiceMode(
      manifest?.website_service_mode,
      websiteUrl,
      normalizeProductMode(manifest?.website_product_mode, websiteUrl)
    ),
...
export function buildAdminRescueBrief({ rescueCase, brand, previousCampaigns = [] }) {
  ...
  const existingProfile = {
    ...
    website_service_mode_available: Boolean(brand?.website_service_mode_available),
    website_service_mode_reason: brand?.website_service_mode_reason || "",
    website_service_source_url: brand?.website_service_source_url || "",
  };
  ...
  const schema = isAnnual
    ...
    : {
        ...
        website_service_mode: {
          available: false,
          reason: "True only when an official source verifies a concrete service offering.",
          source_url: "",
        },
        campaign_opportunities: [
          {
...
TASK
Use ChatGPT web search/browser capabilities to gather VERIFIABLE information from the customer's public website and relevant official sources. Analyze what the company actually sells or provides, its industry, target audience, customer language/market, and whether the website has real product/service pages Spreelo can use. Evaluate website_service_mode independently from product/catalog mode: set it true only when an official page verifies at least one concrete named, described or bookable service, and include the exact supporting source URL. A mixed retailer may legitimately have both products and services. Then create the same type of brand-adapted campaign calendar that Spreelo's automatic analysis creates. Choose only commercially relevant campaigns for the company and market. Do not invent products, services, prices, discounts, warranties, events or other facts.
