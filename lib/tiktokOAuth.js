...
export function isTikTokPerformanceInsightsEnabled() {
  return String(process.env.TIKTOK_ENABLE_PERFORMANCE_INSIGHTS || "").trim().toLowerCase() === "true";
}

export function getTikTokRequestedScopes() {
  return isTikTokPerformanceInsightsEnabled()
    ? [...TIKTOK_SCOPES, ...TIKTOK_PERFORMANCE_SCOPES]
    : [...TIKTOK_SCOPES];
}
  ...
  return {
    ...
    allowPrivateTesting:
      String(process.env.TIKTOK_ALLOW_PRIVATE_TESTING || "")
        .trim()
        .toLowerCase() === "true",
    performanceInsightsEnabled: isTikTokPerformanceInsightsEnabled(),
  };
}
...
export function buildTikTokAuthorizationUrl({ clientKey, redirectUri, state }) {
  const params = new URLSearchParams({
    ...
    response_type: "code",
    scope: getTikTokRequestedScopes().join(","),
