...
export function chooseImageBackground({
  assets,
  profile,
  usageCounts = null,
  variationPenalty = 18,
  ...
  const getUsageCount = (assetId) => {
    if (!assetId || !usageCounts) return 0;
    if (usageCounts instanceof Map) return Number(usageCounts.get(assetId) || 0);
    return Number(usageCounts[assetId] || 0);
  };

  const ranked = activeAssets
    .map((asset) => {
      const scored = scoreImageBackground(asset, profile);
      const usageCount = getUsageCount(asset?.id);
      const appliedVariationPenalty = usageCount * Math.max(0, Number(variationPenalty || 0));
      return {
        asset,
        ...scored,
        baseScore: scored.score,
        usageCount,
        variationPenalty: appliedVariationPenalty,
        score: scored.score - appliedVariationPenalty,
      };
    })
