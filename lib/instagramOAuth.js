...
export function normalizeInstagramRedirectUri(value) {
  return String(value || "")
    .trim()
    .replace(/\/+$/, "");
}
...
export function getInstagramEnv() {
  return {
    ...
    redirectUri: normalizeInstagramRedirectUri(
...
export function createSignedInstagramState({
  userId,
  brandProfileId,
  redirectUri,
  secret,
  ...
  const payload = base64UrlEncode(
    JSON.stringify({
      userId,
      brandProfileId,
      redirectUri: normalizeInstagramRedirectUri(redirectUri),
...
export async function exchangeInstagramCodeForShortToken({
  ...
  const normalizedRedirectUri = normalizeInstagramRedirectUri(redirectUri);
  ...
  console.log("Instagram token exchange request:", {
    ...
    redirectUriLength: normalizedRedirectUri?.length || 0,
    redirectUriSentInBody: true,
  ...
  if (!response.ok) {
    console.error("Instagram token exchange response error:", {
      status: response.status,
      statusText: response.statusText,
      errorType: data?.error_type || data?.error?.type,
      errorCode: data?.code || data?.error?.code,
      errorMessage: data?.error_message || data?.error?.message,
      redirectUri: normalizedRedirectUri,
    });
...
export async function saveInstagramConnection({
  ...
  const connectionPayload = {
    ...
    external_account_id: String(instagramUserId),
